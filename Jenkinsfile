node('maven-slave') {
    ...
    def docker_compose_service = 'operation-platform'
    // 生成 jar 包路径
    def jar_path = 'target/cloud-operation-platform.jar'
    // 开发、测试服务器地址
    def remote_servers = [
        'develop': '192.168.99.125',
        'test': '192.168.99.124']
    ...
    stage('Settings branch params') {
        ...
        if (BRANCH_NAME == 'develop') {
            ...
            docker_image_name = 'microservice/dev/operation-platform'
        } else if (BRANCH_NAME == 'test') {
            // 设置构建保留的最大个数，超过指定的个数则丢弃
            properties([buildDiscarder(logRotator(numToKeepStr: '3'))])
            docker_image_name = 'microservice/test/operation-platform'
        } else if (BRANCH_NAME == 'master') {
            // 设置构建保留的最大个数，超过指定的个数则丢弃
            properties([buildDiscarder(logRotator(numToKeepStr: '7'))])
            docker_image_name = 'microservice/operation-platform'
        } else {
